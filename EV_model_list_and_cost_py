...
m = 30  # number of charging station candidates
...
l_d = {(i, k): rnd.uniform(0, 1) for i in I for k in K}  # lambda of the follower problem
...
p = {(k, t): prob.continuous_var(lb=0, ub=1000000, name="p_{0}_{1}".format(k, t)) for k in K for t in T}
...
con7f = {(i, k, t): prob.add_constraint(
    ct=l_d[i, k] + prob.sum(l * v[i, k, t, l] for l in L) + pi[k, t] + p[k, t] + rho[i] + phi[i, k, t] >= 0,
    ctname="con7f_{0}_{1}_{2}".format(i, k, t)) for i in I for k in K for t in T}

con7g = {(i, k, t): prob.add_constraint(
    ct=l_d[i, k] + prob.sum(l * v[i, k, t, l] for l in L) + p[k, t] + pi[k, t] + rho[i] + phi[i, k, t] - M * (
            1 - x[i, k, t]) <= 0,
